...
function processWaInput(){
  ...
  lines.forEach(line => {
    ...
    // ── Parseur séquentiel universel ─────────────────────────────────────
    // Supporte les 4 formats d'écriture des courses, avec ou sans prix :
    //
    //   Format 1  "Javil  hlib sanecroi khal"          (espaces multiples)
    //   Format 2  une ligne par produit                 (géré par split('\n') en amont)
    //   Format 3  "Javil .hlib .sanecroi .khal"         (points séparateurs)
    //   Format 4  "Javil,hlib ,sanecroi ,khal"          (virgules séparateurs)
    //
    //   Avec prix : "Javil 12 hlib 8 sanecroi 15 khal 5"
    //               "Javil 12, hlib 8, sanecroi 15, khal 5"
    //               "Javil .12 hlib .8 sanecroi .15"
    //
    // Algorithme :
    //   Passe 1 — normaliser tous les séparateurs (,  ;  .) en espace.
    //   Passe 2 — tokeniser sur espaces et parcourir de gauche à droite :
    //     • token numérique  → prix du produit courant → flush & reset
    //     • token textuel    → flush le produit courant (sans prix), nouveau produit
    // ─────────────────────────────────────────────────────────────────────
    ...
    const flushItem = (name, raw, price) => {
      if(!name) return;
      const nameLow = name.toLowerCase().replace(/dh$/i, '').trim();
    ...
    for(const token of tokens){
      ...
      if(isNumeric){
        ...
        // Chiffre orphelin (ex: fragment de numéro de téléphone) → ignorer
      } else {
        // Nouveau mot produit : flush le produit précédent sans prix
        if(pendingName){
